...
typedef union idt_desc_t {
	uint32_t val[2]; 			// if not using the entry struct below... maybe use this? doesn't seem necessary yet
	struct {
		uint16_t offset_15_00;	//Each IDT entry contains, among other things, a pointer to the corresponding interrupt handler function to be run when that interrupt is received.
		uint16_t seg_selector;  // we will always set this to the Kernel CS; kernel handles all exceptions, interrupts, & system calls
		uint8_t reserved4;		// byte of zeroes
		uint32_t reserved3 : 1; // these 4 reserved bits specify the type_attr; either call, interrupt, or trap gate
		uint32_t reserved2 : 1;	// call gate is used for system calls
		uint32_t reserved1 : 1;
		uint32_t size : 1;		// this is the storage segment; set to 0 for interrupt gates, 1 otherwise
		uint32_t reserved0 : 1;
		uint32_t dpl : 2;		// descriptor priveledge level; set to 3 for system calls, 0 otherwise
		uint32_t present : 1;	// set to 0 for unused interrupts, 1 otherwise; not all entries will be used
